...
# Original path: 🧩 Versions_Support/complete/full.R
# Original created: 2025-07-24 18:44:13
# Original modified: 2025-07-28 10:25:24
# Archive rationale: Full application snapshot from the support versions.
...
.card-individual-glass {
  background: rgba(255,255,255,0.17);
  backdrop-filter: blur(10px) saturate(1.5);
  border-radius: 20px;
  box-shadow: 0 4px 32px 0 rgba(80,100,120,0.18);
  padding: 24px 18px 12px 18px;
  min-width: 330px;
  transition: box-shadow 0.2s;
  border: 1.5px solid rgba(180,200,220,0.28);
}
.card-header {
...
.avatar-sexe {
  ...
  border-radius: 60px;
  background: radial-gradient(circle, #e3eaff 40%, #dbe6fb 100%);
  display: flex; align-items: center; justify-content: center;
  box-shadow: 0 0 8px 1px #adc8f0;
  transition: box-shadow 0.18s;
}
.animated-shadow:hover { box-shadow: 0 0 16px 2px #3b82f6; }
.id-main { font-family: 'Montserrat', sans-serif; font-size: 2.2em; color: #254175; }
.badge-proband { background: #96f; color: #fff; border-radius: 8px; padding: 2px 8px; font-size: 0.8em; }
.sex-chip.male { background: #c6e4ff; color: #17528a; }
.sex-chip.female { background: #ffd4e0; color: #b22855; }
.status-chip.deceased { color: #c14444; font-weight: bold; }
.actions-row { display: flex; gap: 18px; justify-content: center; margin-top: 7px; }
.action-btn { border: none; background: #f3f7fa; border-radius: 11px; padding: 11px; font-size: 1.2em; transition: background 0.14s; box-shadow: 0 1px 4px #dbe6fb; }
.action-btn:hover { background: #e4ebf5; box-shadow: 0 2px 8px #badcff; }
.child-btn { ... }
.close-btn { position: absolute; top: 11px; right: 12px; ... }
  ...
  ")),
    ...
    ")),
            ...
            tags$div(
              class = "sex-button-container",
              tags$div(
                class = "sex-button", onclick = "Shiny.setInputValue('deceased', Math.random())",
                HTML('
...
server <- function(input, output, session) {
  ...
  output$sidePanelIndiv <- renderUI({
    ...
    # Style harmonisé pour tous les boutons
    btn_style <- "
    min-width: 90px; min-height: 65px;
  ...
  "
    btn_hover <- "this.style.background='#e4f1ff'; this.style.borderColor='#2684FF';"
    btn_out <- "this.style.background='rgba(38,132,255,0.05)'; this.style.borderColor='#dbe9fa';"

    div(
      style = "
      box-shadow: 0 6px 24px 0 rgba(60,138,220,0.11), 0 1.5px 6px 0 rgba(0,0,0,0.08);
      border-radius: 22px;
      padding: 32px 28px;
      ...
      # Header
      div(
        style = "display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; border-bottom: 1.5px solid #f0f4fa; padding-bottom: 10px;",
        h4(
          HTML(paste0(
            "<span style='font-size:24px;'>🧾</span> Individual <span style='color:#2684FF; font-weight:600;'>", id, "</span>"
          )),
          style = "margin: 0; font-size: 21px; letter-spacing:0.5px;"
        ),
        actionButton(
          "closeSidePanel", "✖",
          style = paste0(btn_style, "width:38px; height:38px; min-width:38px; min-height:38px; font-size:20px; color:#da2222; padding:0;"),
          onmouseover = "this.style.background='#ffe4e4'; this.style.borderColor='#da2222';",
          onmouseout = btn_out
        )
      ),
    ...
    align-items: center; text-align: center; margin-bottom: 20px;",
        actionButton(
          ...
          style = paste0(btn_style, "color:#1976d2; border:1.5px solid #a3d3ff; font-size: 15px; font-weight:600;"),
      ...
      # Zone centrale
      div(
        style = "display: flex; gap: 18px; align-items: center; justify-content:center;",
        # Colonne de gauche
        div(
          ...
          actionButton(
            "brother",
            label = tagList(
              tags$span(style = "font-size: 28px; color:#2684FF; margin-bottom:3px;", "♂️"),
              tags$div("Brother")
            ),
            style = btn_style,
            onmouseover = btn_hover,
          ...
          actionButton(
            "sister",
            label = tagList(
              tags$span(style = "font-size: 28px; color:#e13e90; margin-bottom:3px;", "♀️"),
              tags$div("Sister")
            ),
            style = btn_style,
            onmouseover = btn_hover,
          ...
          actionButton(
            "sib_unknown",
            label = tagList(
              tags$span(style = "font-size: 24px; color:#555; margin-bottom:4px;", "?"),
              tags$div("Unknown")
            ),
            style = btn_style,
            onmouseover = btn_hover,
        ...
        div(
          ...
          actionButton(
            "Twins",
            label = tagList(
              tags$span(style = "font-size: 25px; color:#1976d2; margin-bottom:2px;", "👥"),
              tags$div("Twin")
            ),
            style = btn_style,
            onmouseover = btn_hover,
          ...
          actionButton(
            "Triplets",
            label = tagList(
              tags$span(style = "font-size: 23px; color:#1976d2; margin-bottom:2px;", "3️⃣"),
              tags$div("Triplet")
            ),
            style = btn_style,
            onmouseover = btn_hover,
          ...
          actionButton(
            "Move",
            label = tagList(
              tags$span(style = "font-size: 23px; color:#1976d2; margin-bottom:2px;", "🔄"),
              tags$div("Reorder")
            ),
            style = btn_style,
            onmouseover = btn_hover,
      ...
      # Ligne enfants
      div(
        style = "display: flex; justify-content: center; gap: 14px; margin-top: 16px;",
        actionButton(
          "child_son",
          label = tagList(
            tags$span(HTML('
            <svg width="28" height="28"><rect x="5" y="5" width="18" height="18" fill="none" stroke="#1976d2" stroke-width="2"/></svg>
          ')),
            tags$div("Son", style = "color:#1976d2;")
          ),
          style = btn_style,
          onmouseover = btn_hover,
          onmouseout = btn_out
        ...
        actionButton(
          ...
          label = tagList(
            tags$span(HTML('
            <svg width="28" height="28"><polygon points="14,4 24,14 14,24 4,14" fill="none" stroke="#666" stroke-width="2"/></svg>
          ')),
            tags$div("Unknown", style = "color:#555;")
          ),
          style = btn_style,
          onmouseover = btn_hover,
          onmouseout = btn_out
        ...
        actionButton(
          ...
          label = tagList(
            tags$span(HTML('
            <svg width="28" height="28"><circle cx="14" cy="14" r="9" fill="none" stroke="#e13e90" stroke-width="2"/></svg>
          ')),
            tags$div("Daughter", style = "color:#e13e90;")
          ),
          style = btn_style,
          onmouseover = btn_hover,
          onmouseout = btn_out
        ...
        actionButton(
          "choose_partner",
          label = tagList(
            tags$span(style = "font-size: 22px; color:#3c8dbc;", "👥"),
            tags$div("Partner", style = "color:#3c8dbc;")
          ),
          style = btn_style,
          onmouseover = btn_hover,
          onmouseout = btn_out
      ...
      tags$hr(style = "border-color:#e3ebf5; margin-top:18px;")
    )
  })
